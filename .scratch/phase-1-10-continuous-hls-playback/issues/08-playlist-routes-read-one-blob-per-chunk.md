...
**Status:** resolved — 2026-08-16. Every criterion is met. The last one open was a timing number, closed by [ticket 12](12-the-playlist-route-reads-the-whole-book-per-poll.md)'s deployed measurement: the playlist serves a 4,962-Chunk Book in 0.72s warm, faster than `/api/library`, with the ~0.7s round-trip floor accounting for nearly all of what is left.
...
- [x] The playlist route responds in well under a second on a ~2,000-Chunk Book. _Measured 2026-08-11 against R2: **1.32s warm, 2.4s cold** on a 4,962-Chunk Book — better than the 5.4s this ticket set out to fix, but not "well under a second", and about 0.6s of it was a 1.6 MB read the route did not need. [Ticket 12](12-the-playlist-route-reads-the-whole-book-per-poll.md) removed that read, and this criterion is closed on its 2026-08-16 deployed measurement: **0.72s warm on the same 4,962-Chunk Book**, two and a half times the size the criterion names. Roughly 0.7s of that is round-trip latency from the measuring machine rather than the route: the same run measured `/api/library` — which reads the same 17 KB index and no Book text — at 0.77s, so **the playlist is now the faster of the two** and nothing on this path scales with the Book's text any more. That is the claim the criterion was reaching for; the absolute number is floored by the network._
...
> **Superseded 2026-08-09 by [ticket 04 of phase 1.11](../../phase-1-11-object-storage-migration/issues/04-segment-origin-becomes-configuration.md).** The decision above held only while reads and writes shared a host. On R2 they do not: the app writes to the S3 endpoint and the Listener plays from a Worker on `*.workers.dev`, so a write response cannot yield the origin a segment is played from — the mechanism does not merely become inconvenient, it returns a wrong answer. The origin is now the `SEGMENT_ORIGIN` environment variable ([segmentOrigin.js](../../../app/_lib/segmentOrigin.js)), and its stated objection — "a second thing to get wrong" — is answered by that variable failing loudly at the seam rather than subtly. What survives unchanged is the part this section was really about: URLs are derivable and are still never stored.
>
> Three things below go with it. `storeBase()` is deleted. The `blob:origin` key, the `SET` that rewrote it on every generated Chunk and the pipelined `GET` that fetched it are all gone, so **`writeChunk` pipelines 2 commands per Chunk, not 3** (~4K for a 2,000-Chunk Book) and **the polled path costs 1 command, not 2** — both tables above under-report the current headroom rather than over-reporting it. And the hazard the stored origin carried is dissolved rather than mitigated: there is no longer anything that can go stale when the store moves.
