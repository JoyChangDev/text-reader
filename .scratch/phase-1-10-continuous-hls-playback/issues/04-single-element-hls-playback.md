...
**Status:** resolved — its one remaining item, cross-origin segment fetching on a real device, was re-checked on 2026-08-11 against the Cloudflare Worker that replaced Vercel Blob. See that criterion below.
...
- [x] **Partial.** Cross-origin segment fetching is confirmed here, since this is the first time real Vercel Blob URLs are played (folded in from ticket 01, which dropped its own A′ case). If segments fail to load, the fix is a CORS header on blob responses — the segment format itself is already settled. _Segments do fetch: a real `.mp3` from the live store returns 200 with real MP3 bytes, no CORS header needed, exactly as this ticket predicted. But the fetch is only reliable while the store isn't rate-limiting us, and [ticket 08](08-playlist-routes-read-one-blob-per-chunk.md) is why it often is. Re-check on a device once that lands._ **Re-checked 2026-08-11, and the subject changed underneath it:** the store is no longer Vercel Blob. Segments are served by a Cloudflare Worker on `*.workers.dev`, genuinely cross-origin from the app, and the device runs in [ticket 06](06-verify-growing-playlist-in-background.md) played through them continuously for 691s and 650s — dozens of segment boundaries, no CORS failure, still with no `crossorigin` on the element. The Worker sends `access-control-allow-origin: *` and answers ranges with `206`. Rate limiting is gone with the store that imposed it._
