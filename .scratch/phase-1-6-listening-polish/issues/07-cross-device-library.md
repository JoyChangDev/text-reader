...
- [ ] `libraryService.js` implements `listBooks()`, `addBook({ bookId, title, chunks })`,
...
- [ ] `GET /api/library`, `POST /api/library`, and `PATCH /api/library/[bookId]` routes are
      implemented and tested the same way `chunks/route.test.js` already is
- [ ] `bookLibrary.js` is reimplemented as an async client calling these routes instead of
      `localStorage`, keeping its existing exported function names/shapes
- [ ] Call sites (`BookLibrary.jsx`, `page.jsx`, and `useBookPlayer.js`'s resume-index
      persistence effect) are updated for the now-async calls
- [ ] A book uploaded from one browser/device appears in the library list and resumes from
      the correct position when opened from a different browser/device
- [ ] `bookLibrary.test.js` is rewritten against the fetch-based client (mocked fetch);
      `libraryService.test.js` covers the persistence logic against a faked storage client
- [ ] `listenerSettings.js` (device-scoped voice/speed prefs) is untouched — only Library
