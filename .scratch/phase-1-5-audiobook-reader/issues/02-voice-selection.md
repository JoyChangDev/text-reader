...
- [ ] A new Listener settings store (per ADR 0001, `listenerSettings.js`) persists a
      `voice` field in its own `localStorage` key, separate from `bookLibrary.js`'s
      per-book records
- [ ] The picker offers all 3 available zh-TW voices: `zh-TW-HsiaoChenNeural` (current
      default), `zh-TW-YunJheNeural`, `zh-TW-HsiaoYuNeural`
- [ ] Defaults to `zh-TW-HsiaoChenNeural` when no preference has been stored yet,
      matching today's hardcoded behavior
- [ ] A minimal Select control is added to the current `AudioPlayer.jsx` — no redesign
      layout work here, that's a separate ticket
- [ ] The selected voice is threaded from the UI through `POST /api/audio-chunks` →
      `generateAudioForChunk` → `getOrGenerateAudio`, replacing the hardcoded
      `DEFAULT_VOICE` passthrough in `audioGenerationService.js`
- [ ] Changing the voice is prospective only: already-generated/cached chunks (keyed by
      ...
      voice
- [ ] Unit tests cover the settings store (read/write/default) and the threading of
