...
describe('useVoicePreview / VoicePreviewButton', () => {
  ...
  test('offers a preview button for every available voice', () => {
    ...
    expect(screen.getByRole('button', { name: /試聽 hsiao-chen/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /試聽 yun-jhe/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /試聽 hsiao-yu/i })).toBeInTheDocument();
  });
  ...
  test('clicking the same preview button again stops the sample clip', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /試聽 hsiao-yu/i }));
    await screen.findByRole('button', { name: /停止 hsiao-yu/i });

    fireEvent.click(screen.getByRole('button', { name: /停止 hsiao-yu/i }));

    expect(window.HTMLMediaElement.prototype.pause).toHaveBeenCalledTimes(1);
    expect(await screen.findByRole('button', { name: /試聽 hsiao-yu/i })).toBeInTheDocument();
  });
  ...
  test('previewing a different voice while one is already playing switches to the new clip', async () => {
    ...
    await screen.findByRole('button', { name: /停止 yun-jhe/i });

    fireEvent.click(screen.getByRole('button', { name: /試聽 hsiao-yu/i }));

    const previewAudioEl = screen.getByTestId('voice-preview-audio');
    expect(previewAudioEl.src).toContain('/voice-samples/zh-TW-HsiaoYuNeural.mp3');
    expect(await screen.findByRole('button', { name: /停止 hsiao-yu/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /試聽 yun-jhe/i })).toBeInTheDocument();
  });
  ...
  test('resets to the preview label once the sample clip finishes on its own', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /試聽 hsiao-chen/i }));
    await screen.findByRole('button', { name: /停止 hsiao-chen/i });

    fireEvent.ended(screen.getByTestId('voice-preview-audio'));

    expect(await screen.findByRole('button', { name: /試聽 hsiao-chen/i })).toBeInTheDocument();
  });
  ...
  test("clicking preview inside a label does not activate that label's associated control", () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /試聽/i }));
