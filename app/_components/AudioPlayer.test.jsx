...
describe('AudioPlayer voice preview samples', () => {
  ...
  test('plays the static sample clip for a voice without touching the persisted selection or generating audio', async () => {
    render(
      <ChakraProvider>
        <AudioPlayer bookId="book-preview" chunks={chunks} />
      </ChakraProvider>,
    );

    await waitFor(() => expect(global.fetch).toHaveBeenCalledTimes(3));
    const fetchCallsBeforePreview = global.fetch.mock.calls.length;

    const previewButton = await screen.findByRole('button', { name: /preview yun-jhe/i });
    ...
    // Previewing is non-committal: the persisted/select voice is untouched, and no
    // edge-tts or /api/audio-chunks call was made just to preview.
    expect(getListenerSettings().voice).toBe('zh-TW-HsiaoChenNeural');
    expect(screen.getByLabelText(/narration voice/i)).toHaveValue('zh-TW-HsiaoChenNeural');
    expect(global.fetch.mock.calls.length).toBe(fetchCallsBeforePreview);
  ...
  test('clicking the same preview button again stops the sample clip', async () => {
    render(
      <ChakraProvider>
        <AudioPlayer bookId="book-preview-2" chunks={chunks} />
      </ChakraProvider>,
    );

    const previewButton = await screen.findByRole('button', { name: /preview hsiao-yu/i });
    fireEvent.click(previewButton);
  ...
  test('previewing a different voice while one is already playing switches to the new clip', async () => {
    render(
      <ChakraProvider>
        <AudioPlayer bookId="book-preview-3" chunks={chunks} />
      </ChakraProvider>,
    );

    fireEvent.click(await screen.findByRole('button', { name: /preview yun-jhe/i }));
    await screen.findByRole('button', { name: /stop yun-jhe/i });

    fireEvent.click(await screen.findByRole('button', { name: /preview hsiao-yu/i }));
  ...
  test('resets to the preview label once the sample clip finishes on its own', async () => {
    render(
      <ChakraProvider>
        <AudioPlayer bookId="book-preview-4" chunks={chunks} />
      </ChakraProvider>,
    );

    fireEvent.click(await screen.findByRole('button', { name: /preview hsiao-chen/i }));
