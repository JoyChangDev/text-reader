...
- 利用開發 `text-reader` 這個專案的空檔零散學習，沒有固定排程 —— 每堂課都要短、能一次坐下來學完、有單一明確的收穫。
- 截至 2026-07-15，完全沒有任何測試經驗（不論哪種語言）—— 不要假設對斷言、mock、測試框架這些概念有任何先備知識。
- 已經對 React/JS/Next.js 很熟練（就是靠這些技術做出這個專案的）—— 完全不需要重新教 React 基礎。
- 希望每堂課盡量同時帶來兩種價值：(1) 可轉移的測試能力，(2) `text-reader` 測試套件的實質進度。只要範圍合適，優先設計會產出專案真的會留下來的程式碼的練習，而不是用完即丟的玩具範例。
